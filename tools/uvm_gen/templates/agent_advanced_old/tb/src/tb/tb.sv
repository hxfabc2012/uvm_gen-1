...
/**
 * Module instantiating the {{ name_proper }} UVM Agent interfaces, a clock % reset generating interface and a "DUT".
 */
module uvmt_{{ name }}_st_tb;
   ...
   uvma_{{ name }}_if  {{ mode_1 }}_if (.clk(gen_if.clk), .reset_n(gen_if.reset_n)); ///< Active agent interface
   uvma_{{ name }}_if  {{ mode_2 }}_if(.clk(gen_if.clk), .reset_n(gen_if.reset_n)); ///< Passive agent interface
