...
/**
 * Sends a fixed number of completely random sequence items to the {{ mode_1 }} sequencer.
 */
class uvme_{{ name }}_st_rand_stim_vseq_c extends uvme_{{ name }}_st_base_vseq_c;
   ...
   /**
    * Creates and sends #num_items of completely random uvma_{{ name }}_seq_item_c instances to the {{ mode_1 }} sequencer.
    */
...
task uvme_{{ name }}_st_rand_stim_vseq_c::body();
   ...
   for (int unsigned ii=0; ii<num_items; ii++) begin
      ...
      `uvm_do_on(seq_item, p_sequencer.{{ mode_1 }}_sequencer)
   end
