...
`ifndef __UVMA_{{ name_uppercase }}_IDLE_VSEQ_SV__
`define __UVMA_{{ name_uppercase }}_IDLE_VSEQ_SV__
...
task uvma_{{ name }}_idle_vseq_c::body();
   ...
   `uvm_info("{{ name_uppercase }}_IDLE_VSEQ", "Idle virtual sequence has started", UVM_HIGH)
   case (cfg.drv_mode)
      UVMA_{{ name_uppercase }}_DRV_MODE_MSTR: do_idle_mstr();
      UVMA_{{ name_uppercase }}_DRV_MODE_SLV : do_idle_slv ();
   endcase
...
task uvma_{{ name }}_idle_vseq_c::do_idle_mstr_a();
   ...
   case (cfg.drv_idle)
      UVMA_{{ name_uppercase }}_DRV_IDLE_ZEROS: begin
         `uvm_rand_send_pri_with(mstr_a_seq_item, `UVMA_{{ name_uppercase }}_MSTR_A_IDLE_SEQ_ITEM_PRI, {
            req     == 0;
            addr    == 0;
            we      == 0;
            be      == 0;
      ...
      UVMA_{{ name_uppercase }}_DRV_IDLE_RANDOM: begin
         `uvm_rand_send_pri_with(mstr_a_seq_item, `UVMA_{{ name_uppercase }}_MSTR_A_IDLE_SEQ_ITEM_PRI, {
            req == 0;
         })
      end
...
task uvma_{{ name }}_idle_vseq_c::do_idle_mstr_r();
   ...
   `uvm_rand_send_pri_with(mstr_r_seq_item, `UVMA_{{ name_uppercase }}_MSTR_R_IDLE_SEQ_ITEM_PRI, {
      rready == 0;
   })
...
task uvma_{{ name }}_idle_vseq_c::do_idle_slv_a();
   ...
   `uvm_rand_send_pri_with(slv_a_seq_item, `UVMA_{{ name_uppercase }}_SLV_A_IDLE_SEQ_ITEM_PRI, {
      gnt == 0;
   })
...
task uvma_{{ name }}_idle_vseq_c::do_idle_slv_r();
   ...
   case (cfg.drv_idle)
      UVMA_{{ name_uppercase }}_DRV_IDLE_ZEROS: begin
         `uvm_rand_send_pri_with(slv_r_seq_item, `UVMA_{{ name_uppercase }}_SLV_R_IDLE_SEQ_ITEM_PRI, {
            rvalid    == 0;
            rdata     == 0;
            err       == 0;
            ruser     == 0;
