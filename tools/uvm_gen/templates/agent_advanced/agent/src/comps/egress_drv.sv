...
`ifndef __UVMA_${name_uppercase}_MSTR_A_DRV_SV__
`define __UVMA_${name_uppercase}_MSTR_A_DRV_SV__
...
function void uvma_${name}_mstr_a_drv_c::build_phase(uvm_phase phase);
   ...
   if (cfg == null) begin
      `uvm_fatal("${name_uppercase}_MSTR_A_DRV", "Configuration handle is null")
   end
   uvm_config_db#(uvma_${name}_cfg_c)::set(this, "*", "cfg", cfg);
   
   void'(uvm_config_db#(uvma_${name}_cntxt_c)::get(this, "", "cntxt", cntxt));
   if (cntxt == null) begin
      `uvm_fatal("${name_uppercase}_MSTR_A_DRV", "Context handle is null")
   end
...
task uvma_${name}_mstr_a_drv_c::run_phase(uvm_phase phase);
   ...
   if (cfg.enabled && cfg.is_active && (cfg.drv_mode == UVMA_${name_uppercase}_DRV_MODE_MSTR)) begin
      forever begin
...
function void uvma_${name}_mstr_a_drv_c::process_req(ref uvma_${name}_mstr_a_seq_item_c req);
   ...
   req.cfg = cfg;
   `uvm_info("${name_uppercase}_MSTR_A_DRV", $sformatf("Got new req from the sequencer:\n%s", req.sprint()), UVM_HIGH)
...
`endif // __UVMA_${name_uppercase}_MSTR_A_DRV_SV__
