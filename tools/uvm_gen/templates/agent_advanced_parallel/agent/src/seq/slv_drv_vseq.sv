...
`ifndef __UVMA_{{ upper(name) }}_SLV_DRV_VSEQ_SV__
`define __UVMA_{{ upper(name) }}_SLV_DRV_VSEQ_SV__
...
task uvma_{{ name }}_slv_drv_vseq_c::drive_a();
   ...
   forever begin
      ...
      if ($urandom_range(0,1)) begin
         ...
         `uvm_rand_send_pri_with(slv_a_seq_item, `UVMA_{{ upper(name) }}_SLV_DRV_SEQ_ITEM_PRI, {
            gnt == 1'b1;
         })
      end
      else begin
...
task uvma_{{ name }}_slv_drv_vseq_c::response_loop();
   ...
   forever begin
      ...
      while (cntxt.drv_slv_outstanding_q.size()) begin
         ...
         if (!handled) begin
            `uvm_warning("{{ upper(name) }}_SLV_DRV_VSEQ", $sformatf("Request from MSTR not handled:\n%s", trn.sprint()))
         end
      end
   end
