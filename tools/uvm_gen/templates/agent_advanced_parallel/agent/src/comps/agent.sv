...
class uvma_{{ name }}_agent_c extends uvml_agent_c;
   ...
   uvm_analysis_port      #(uvma_{{ name }}_mon_trn_c        )  mon_trn_ap   ; ///< TODO Describe uvma_{{ name }}_agent_c::mon_trn_ap
   uvm_analysis_port      #(uvma_{{ name }}_mstr_a_seq_item_c)  drv_mstr_a_ap; ///< TODO Describe uvma_{{ name }}_agent_c::drv_mstr_a_ap
   uvm_analysis_port      #(uvma_{{ name }}_mstr_r_seq_item_c)  drv_mstr_r_ap; ///< TODO Describe uvma_{{ name }}_agent_c::drv_mstr_r_ap
   uvm_analysis_port      #(uvma_{{ name }}_slv_a_seq_item_c )  drv_slv_a_ap ; ///< TODO Describe uvma_{{ name }}_agent_c::drv_slv_a_ap
   uvm_analysis_port      #(uvma_{{ name }}_slv_r_seq_item_c )  drv_slv_r_ap ; ///< TODO Describe uvma_{{ name }}_agent_c::drv_slv_r_ap
   uvm_analysis_port      #(uvma_{{ name }}_mstr_a_mon_trn_c )  mon_mstr_a_ap; ///< TODO Describe uvma_{{ name }}_agent_c::mon_mstr_a_ap
   uvm_analysis_port      #(uvma_{{ name }}_mstr_r_mon_trn_c )  mon_mstr_r_ap; ///< TODO Describe uvma_{{ name }}_agent_c::mon_mstr_r_ap
   uvm_analysis_port      #(uvma_{{ name }}_slv_a_mon_trn_c  )  mon_slv_a_ap ; ///< TODO Describe uvma_{{ name }}_agent_c::mon_slv_a_ap
   uvm_analysis_port      #(uvma_{{ name }}_slv_r_mon_trn_c  )  mon_slv_r_ap ; ///< TODO Describe uvma_{{ name }}_agent_c::mon_slv_r_ap
   ...
   /**
    * TODO Describe uvma_{{ name }}_agent_c::start_mstr_drv_vseq()
    */
   extern task start_mstr_drv_vseq();

   /**
    * TODO Describe uvma_{{ name }}_agent_c::start_slv_drv_vseq()
    */
...
task uvma_{{ name }}_agent_c::run_phase(uvm_phase phase);
   ...
   if (cfg.enabled) begin
      ...
      if (cfg.is_active) begin
         ...
         case (cfg.drv_mode)
            UVMA_{{ upper(name) }}_DRV_MODE_MSTR: start_mstr_drv_vseq();
            UVMA_{{ upper(name) }}_DRV_MODE_SLV : start_slv_drv_vseq ();
...
task uvma_{{ name }}_agent_c::start_mstr_drv_vseq();
   ...
   if (!$cast(cntxt.mstr_drv_vseq, temp_obj)) begin
      `uvm_fatal("{{ upper(name) }}_AGENT", $sformatf("Could not cast 'temp_obj' (%s) to 'cntxt.mstr_drv_vseq' (%s)", $typename(temp_obj), $typename(cntxt.mstr_drv_vseq)))
   end

   if (!cntxt.mstr_drv_vseq.randomize()) begin
      `uvm_fatal("{{ upper(name) }}_AGENT", "Failed to randomize cntxt.mstr_drv_vseq")
   end
...
task uvma_{{ name }}_agent_c::start_slv_drv_vseq();
   ...
   if (!$cast(cntxt.slv_drv_vseq, temp_obj)) begin
      `uvm_fatal("{{ upper(name) }}_AGENT", $sformatf("Could not cast 'temp_obj' (%s) to 'cntxt.slv_drv_vseq' (%s)", $typename(temp_obj), $typename(cntxt.slv_drv_vseq)))
   end

   if (!cntxt.slv_drv_vseq.randomize()) begin
      `uvm_fatal("{{ upper(name) }}_AGENT", "Failed to randomize cntxt.slv_drv_vseq")
   end
