...
task uvma_{{ name }}_mon_c::sample_mstr_a_trn(output uvma_{{ name }}_mstr_a_mon_trn_c trn);
   ...
   @(mp_a.mon_a_cb);
   `uvm_info("{{ upper(name) }}_MON", "Sampling MSTR Channel A transaction", UVM_DEBUG)
...
task uvma_{{ name }}_mon_c::sample_mstr_r_trn(output uvma_{{ name }}_mstr_r_mon_trn_c trn);
   ...
   @(mp_r.mon_r_cb);
   `uvm_info("{{ upper(name) }}_MON", "Sampling MSTR Channel R transaction", UVM_HIGH)
...
task uvma_{{ name }}_mon_c::sample_slv_a_trn(output uvma_{{ name }}_slv_a_mon_trn_c trn);
   ...
   @(mp_a.mon_a_cb);
   `uvm_info("{{ upper(name) }}_MON", "Sampling SLV Channel A transaction", UVM_DEBUG)
...
task uvma_{{ name }}_mon_c::sample_slv_r_trn(output uvma_{{ name }}_slv_r_mon_trn_c trn);
   ...
   @(mp_r.mon_r_cb);
   `uvm_info("{{ upper(name) }}_MON", "Sampling SLV Channel R transaction", UVM_DEBUG)
...
function void uvma_{{ name }}_mon_c::process_mstr_a_trn(ref uvma_{{ name }}_mstr_a_mon_trn_c trn);
   ...
   `uvm_info("{{ upper(name) }}_MON", $sformatf("Sampled MSTR channel A transaction from the virtual interface:\n%s", trn.sprint()), UVM_HIGH)
...
function void uvma_{{ name }}_mon_c::process_mstr_r_trn(ref uvma_{{ name }}_mstr_r_mon_trn_c trn);
   ...
   trn.set_initiator(this);
   trn.set_timestamp_end($realtime());
   `uvm_info("{{ upper(name) }}_MON", $sformatf("Sampled MSTR channel R transaction from the virtual interface:\n%s", trn.sprint()), UVM_HIGH)
...
function void uvma_{{ name }}_mon_c::process_slv_a_trn(ref uvma_{{ name }}_slv_a_mon_trn_c trn);
   ...
   trn.set_initiator(this);
   trn.set_timestamp_end($realtime());
   `uvm_info("{{ upper(name) }}_MON", $sformatf("Sampled SLV channel A transaction from the virtual interface:\n%s", trn.sprint()), UVM_HIGH)
...
function void uvma_{{ name }}_mon_c::process_slv_r_trn(ref uvma_{{ name }}_slv_r_mon_trn_c trn);
   ...
   trn.set_initiator(this);
   trn.set_timestamp_end($realtime());
   `uvm_info("{{ upper(name) }}_MON", $sformatf("Sampled SLV channel R transaction from the virtual interface:\n%s", trn.sprint()), UVM_HIGH)
