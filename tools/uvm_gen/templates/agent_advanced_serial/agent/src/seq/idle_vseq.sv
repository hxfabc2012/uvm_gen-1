...
task uvma_{{ name }}_idle_vseq_c::do_idle_tx();
   ...
   uvma_{{ name }}_seq_item_c  {{ tx }}_idle;
   
   forever begin
      `uvm_create_on({{ tx }}_idle, p_sequencer.frame_sequencer)
      {{ tx }}_idle.direction = UVMA_{{ upper(name) }}_DIRECTION_{{ upper(tx) }};
      {{ tx }}_idle.q_sync    = 2'b01;
      {{ tx }}_idle.i_sync    = 2'b10;
      {{ tx }}_idle.q0        = 0;
      {{ tx }}_idle.q_data    = 0;
      {{ tx }}_idle.{{ tx }}_ctrl   = 0;
      {{ tx }}_idle.i_data    = 0;
      {{ tx }}_idle.is_idle   = 1;
      `uvm_send_pri({{ tx }}_idle, `UVMA_{{ upper(name) }}_FRAME_IDLE_PRI)
   end
...
task uvma_{{ name }}_idle_vseq_c::do_idle_rx();
   ...
   uvma_{{ name }}_seq_item_c  {{ rx }}_idle;
   
   forever begin
      `uvm_create_on({{ rx }}_idle, p_sequencer.frame_sequencer)
      {{ rx }}_idle.direction = UVMA_{{ upper(name) }}_DIRECTION_{{ upper(rx) }};
      {{ rx }}_idle.q_sync    = 2'b01;
      {{ rx }}_idle.i_sync    = 2'b10;
      {{ rx }}_idle.q0        = 0;
      {{ rx }}_idle.q_data    = 0;
      {{ rx }}_idle.{{ tx }}_ctrl   = 0;
      {{ rx }}_idle.i_data    = 0;
      {{ rx }}_idle.is_idle   = 1;
      `uvm_send_pri({{ rx }}_idle, `UVMA_{{ upper(name) }}_FRAME_IDLE_PRI)
   end
