...
class uvma_{{ name }}_mon_vseq_c extends uvma_{{ name }}_base_vseq_c;
    ...
    */
   extern virtual function bit monitor_fsm(uvma_{{ name }}_mon_fsm_cntxt_c fsm_cntxt, string direction);
...
task uvma_{{ name }}_mon_vseq_c::monitor_tx();
   ...
   uvma_{{ name }}_{{ tx }}_mon_trn_c  serial_trn  ;
   uvma_{{ name }}_mon_trn_c     frame_trn   ;
   bit                       sample_frame;
   bit                       frame_data[];
   
   forever begin
      get_{{ tx }}_mon_trn(serial_trn);
      cntxt.mon_fsm_{{ tx }}_cntxt.{{ rx }}_data.push_back(serial_trn.txdp);
      sample_frame = monitor_fsm(cntxt.mon_fsm_{{ tx }}_cntxt, "TX");
      if (sample_frame) begin
         frame_trn = uvma_{{ name }}_mon_trn_c::type_id::create("frame_trn");
         void'(frame_trn.unpack(cntxt.mon_fsm_{{ tx }}_cntxt.{{ rx }}_frame));
...
task uvma_{{ name }}_mon_vseq_c::monitor_rx();
   ...
   uvma_{{ name }}_{{ rx }}_mon_trn_c  serial_trn  ;
   uvma_{{ name }}_mon_trn_c     frame_trn   ;
   bit                       sample_frame;
   
   forever begin
      get_{{ rx }}_mon_trn(serial_trn);
      cntxt.mon_fsm_{{ rx }}_cntxt.{{ rx }}_data.push_back(serial_trn.rxd24p); // TODO How to know which data to use?
      sample_frame = monitor_fsm(cntxt.mon_fsm_{{ rx }}_cntxt, "RX");
      if (sample_frame) begin
         frame_trn = uvma_{{ name }}_mon_trn_c::type_id::create("frame_trn");
         void'(frame_trn.unpack(cntxt.mon_fsm_{{ rx }}_cntxt.{{ rx }}_frame));
         if (!frame_trn.is_idle()) begin
            `uvml_hrtbt_owner(p_sequencer)
...
function bit uvma_{{ name }}_mon_vseq_c::monitor_fsm(uvma_{{ name }}_mon_fsm_cntxt_c fsm_cntxt, string direction);
   ...
   {{ rx }}_bit = fsm_cntxt.{{ rx }}_data.pop_front();
   `uvm_info({"{{ upper(name) }}_MON_VSEQ_", direction}, $sformatf("received bit = %0b", {{ rx }}_bit), UVM_DEBUG)
   ...
   case (fsm_cntxt.state)
      UVMA_{{ upper(name) }}_MON_FSM_INIT: begin
         ...
         for (int i = 31; i > 0; i--) begin
            fsm_cntxt.{{ rx }}_frame[i] = fsm_cntxt.{{ rx }}_frame[i-1];
         end
         fsm_cntxt.{{ rx }}_frame[0] = {{ rx }}_bit;
         fsm_cntxt.startup_counter++;
         `uvm_info({"{{ upper(name) }}_MON_VSEQ_", direction}, $sformatf("%s - startup_{{ rx }}_count = %0d", fsm_cntxt.state.name(), fsm_cntxt.startup_counter), UVM_DEBUG)
      ...
      UVMA_{{ upper(name) }}_MON_FSM_STARTUP: begin
         ...
         if (fsm_cntxt.{{ rx }}_frame[31] == 0) begin
         ...
         foreach (fsm_cntxt.{{ rx }}_frame[ii]) begin
            frame[ii] = fsm_cntxt.{{ rx }}_frame[ii];
         end
         `uvm_info({"{{ upper(name) }}_MON_VSEQ_", direction}, $sformatf("%s - {{ rx }}_frame = %b", fsm_cntxt.state.name(), frame), UVM_DEBUG)

         if (fsm_cntxt.zero_cnt >= 32) begin
            if (fsm_cntxt.{{ rx }}_frame[31] == 1 && fsm_cntxt.{{ rx }}_frame[30] == 0 && fsm_cntxt.{{ rx }}_frame[15] == 0 && fsm_cntxt.{{ rx }}_frame[14] == 1) begin
               next_state = UVMA_{{ upper(name) }}_MON_FSM_SYNCED;
               sample_frame = 1;
               fsm_cntxt.{{ rx }}_index = 0;
      ...
      UVMA_{{ upper(name) }}_MON_FSM_SYNCED: begin
         ...
         fsm_cntxt.{{ rx }}_frame[0] = {{ rx }}_bit;
         fsm_cntxt.{{ rx }}_index = fsm_cntxt.{{ rx }}_index + 1;
         next_state = UVMA_{{ upper(name) }}_MON_FSM_SYNCED;
         sample_frame = 0;
         `uvm_info({"{{ upper(name) }}_MON_VSEQ_", direction}, $sformatf("%s - {{ rx }}_index = %0d", fsm_cntxt.state.name(), fsm_cntxt.{{ rx }}_index), UVM_DEBUG)
         foreach (fsm_cntxt.{{ rx }}_frame[ii]) begin
            frame[ii] = fsm_cntxt.{{ rx }}_frame[ii];
         end
         `uvm_info({"{{ upper(name) }}_MON_VSEQ_", direction}, $sformatf("%s - {{ rx }}_frame = %b", fsm_cntxt.state.name(), frame), UVM_DEBUG)

         if (fsm_cntxt.{{ rx }}_index == 32) begin
            fsm_cntxt.{{ rx }}_index = 0;
            if (fsm_cntxt.{{ rx }}_frame[31] == 1 && fsm_cntxt.{{ rx }}_frame[30] == 0 && fsm_cntxt.{{ rx }}_frame[15] == 0 && fsm_cntxt.{{ rx }}_frame[14] == 1) begin
               next_state = UVMA_{{ upper(name) }}_MON_FSM_SYNCED;
               sample_frame = 1;
