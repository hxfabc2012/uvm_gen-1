...
`ifndef __UVMA_{{ upper(name) }}_{{ upper(rx) }}_DRV_VSEQ_SV__
`define __UVMA_{{ upper(name) }}_{{ upper(rx) }}_DRV_VSEQ_SV__


/**
 * TODO Describe uvma_{{ name }}_{{ rx }}_drv_vseq_c
 */
class uvma_{{ name }}_{{ rx }}_drv_vseq_c extends uvma_{{ name }}_base_vseq_c;
   ...
   /**
    * TODO Describe uvma_{{ name }}_{{ rx }}_drv_vseq_c::body()
    */
   extern virtual task body();

   /**
    * TODO Describe uvma_{{ name }}_{{ rx }}_drv_vseq_c::wait_for_reset()
    */
   extern task wait_for_reset();

   /**
    * TODO Describe uvma_{{ name }}_{{ rx }}_drv_vseq_c::post_reset_init()
    */
   extern task post_reset_init();

   /**
    * TODO Describe uvma_{{ name }}_{{ rx }}_drv_vseq_c::drv_loop()
    */
   extern task drv_loop();

   /**
    * TODO Describe uvma_{{ name }}_{{ rx }}_drv_vseq_c::reset_trigger()
    */
   extern task reset_trigger();

   /**
    * TODO Describe uvma_{{ name }}_{{ rx }}_drv_vseq_c::process_seq_item()
    */
   extern function void process_seq_item(ref uvma_{{ name }}_seq_item_c seq_item);

   /**
    * TODO Describe uvma_{{ name }}_{{ rx }}_drv_vseq_c::drive()
    */
...
task uvma_{{ name }}_{{ rx }}_drv_vseq_c::body();
   ...
   `uvm_info("{{ upper(name) }}_{{ upper(rx) }}_DRV_VSEQ", "RX Driver Virtual Sequence has started", UVM_HIGH)
   forever begin
      fork
         begin : main
            wait_for_reset ();
...
task uvma_{{ name }}_{{ rx }}_drv_vseq_c::wait_for_reset();
   ...
   `uvm_info("{{ upper(name) }}_{{ upper(rx) }}_DRV_VSEQ", "Waiting for post_reset", UVM_HIGH)
   wait (cnrxt.reset_state == UVML_RESET_STATE_POST_RESET);
   `uvm_info("{{ upper(name) }}_{{ upper(rx) }}_DRV_VSEQ", "Done waiting for post_reset", UVM_HIGH)
...
task uvma_{{ name }}_{{ rx }}_drv_vseq_c::post_reset_init();
   ...
   `uvm_info("{{ upper(name) }}_{{ upper(rx) }}_DRV_VSEQ", {"Starting training sequence:\n", training_vseq.sprint()}, UVM_HIGH)
   `uvm_do_on(training_vseq, p_sequencer)
   `uvm_info("{{ upper(name) }}_{{ upper(rx) }}_DRV_VSEQ", {"Finished training sequence:\n", training_vseq.sprint()}, UVM_HIGH)
...
task uvma_{{ name }}_{{ rx }}_drv_vseq_c::drv_loop();
   ...
   forever begin
      `uvm_info("{{ upper(name) }}_{{ upper(rx) }}_DRV_VSEQ", "Waiting for next seq_item", UVM_DEBUG)
      p_sequencer.get_next_item    (seq_item);
      `uvm_info("{{ upper(name) }}_{{ upper(rx) }}_DRV_VSEQ", $sformatf("Received seq_item:\n%s", seq_item.sprint()), UVM_DEBUG)
      process_seq_item             (seq_item);
      drive                        (seq_item);
      p_sequencer.seq_item_ap.write(seq_item);
      `uvm_info("{{ upper(name) }}_{{ upper(rx) }}_DRV_VSEQ", {"Done with seq_item:\n", seq_item.sprint()}, UVM_DEBUG)
...
task uvma_{{ name }}_{{ rx }}_drv_vseq_c::reset_trigger();
   ...
   wait (cnrxt.reset_state == UVML_RESET_STATE_POST_RESET);
   wait (cnrxt.reset_state != UVML_RESET_STATE_POST_RESET);
   `uvm_info("{{ upper(name) }}_{{ upper(rx) }}_DRV_VSEQ", "Mid-sim reset detected. Resetting main loop, this may kill some sequences.", UVM_NONE)
...
function void uvma_{{ name }}_{{ rx }}_drv_vseq_c::process_seq_item(ref uvma_{{ name }}_seq_item_c seq_item);
   ...
   seq_item.cfg       = cfg;
   seq_item.direction = UVMA_{{ upper(name) }}_DIRECTION_{{ upper(rx) }};
   `uvm_info("{{ upper(name) }}_{{ upper(rx) }}_DRV_VSEQ", $sformatf("Processed seq_item:\n%s", seq_item.sprint()), UVM_DEBUG)
...
task uvma_{{ name }}_{{ rx }}_drv_vseq_c::drive(ref uvma_{{ name }}_seq_item_c seq_item);
   ...
   `uvm_info("{{ upper(name) }}_{{ upper(rx) }}_DRV_VSEQ", $sformatf("Driving %0d bits of serial data", num_seq_item_bits), UVM_DEBUG)
   ...
   `uvm_info("{{ upper(name) }}_{{ upper(rx) }}_DRV_VSEQ", $sformatf("Finished driving %0d bits of serial data", num_seq_item_bits), UVM_DEBUG)
...
`endif // __UVMA_{{ upper(name) }}_{{ upper(rx) }}_DRV_VSEQ_SV__
