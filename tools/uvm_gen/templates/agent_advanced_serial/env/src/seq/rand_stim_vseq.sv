...
`ifndef __UVME_{{ upper(name) }}_ST_RAND_STIM_VSEQ_SV__
`define __UVME_{{ upper(name) }}_ST_RAND_STIM_VSEQ_SV__
...
task uvme_{{ name }}_st_rand_stim_vseq_c::body();
   ...
   `uvm_info("{{ upper(name }}_ST_RAND_STIM_VSEQ", "Starting sequence", UVM_MEDIUM)
   fork
      begin : {{ tx }}
         `uvm_info("{{ upper(name }}_ST_RAND_STIM_VSEQ_{{ upper(tx) }}", "Starting stimulus", UVM_LOW)
         for (int unsigned ii=0; ii<num_{{ tx }}_seq_items; ii++) begin
            `uvm_info("{{ upper(name }}_ST_RAND_STIM_VSEQ_{{ upper(tx) }}", $sformatf("Starting item %0d of %0d", (ii+1), num_{{ tx }}_seq_items), UVM_HIGH)
            `uvm_do_on_pri({{ tx }}_seq_item, p_sequencer.{{ mode_1 }}_vsequencer, sqr_priority)
            `uvm_info("{{ upper(name }}_ST_RAND_STIM_VSEQ_{{ upper(tx) }}", $sformatf("Finished item %0d of %0d", (ii+1), num_{{ tx }}_seq_items), UVM_HIGH)
         end
      end
      begin : {{ rx }}
         `uvm_info("{{ upper(name }}_ST_RAND_STIM_VSEQ_{{ upper(rx) }}", "Starting stimulus", UVM_LOW)
         for (int unsigned ii=0; ii<num_{{ rx }}_seq_items; ii++) begin
            `uvm_info("{{ upper(name }}_ST_RAND_STIM_VSEQ_{{ upper(rx) }}", $sformatf("Starting item %0d of %0d", (ii+1), num_{{ rx }}_seq_items), UVM_HIGH)
            `uvm_do_on_pri({{ rx }}_seq_item, p_sequencer.{{ mode_2 }}_vsequencer, sqr_priority)
            `uvm_info("{{ upper(name }}_ST_RAND_STIM_VSEQ_{{ upper(rx) }}", $sformatf("Finished item %0d of %0d", (ii+1), num_{{ rx }}_seq_items), UVM_HIGH)
         end
      end
   join
...
`endif // __UVME_{{ upper(name) }}_ST_RAND_STIM_VSEQ_SV__
